"""Prescription OCR microservice — CR-001 Sprint OCR-01 skeleton, now with
real text recognition (Sprint OCR-03).
...
MockOCRProvider (Sprint OCR-01) stays registered and remains the
*default* provider — every existing caller (NestJS's worker, the e2e
test stub) keeps working unchanged. PaddleOCRProvider (Sprint OCR-03) is
registered alongside it, selectable per-call via the existing optional
`provider` field on /v1/detect-and-recognize, or by making it the
process default via OCR_PROVIDER (see app/config.py). Switching the
NestJS worker itself over to PaddleOCR is a separate, later step.
"""

from __future__ import annotations

# Must be the very first project import: app.config sets several
# PADDLE_PDX_* environment variables that paddleocr/paddlex read into
# module-level constants at *their* import time (font source, model
# cache location, model download source) — see app/config.py's docstring.
...
registry.register(MockOCRProvider(), default=(_config.RUNTIME.provider != "paddleocr"))
registry.register(PaddleOCRProvider(), default=(_config.RUNTIME.provider == "paddleocr"))


@asynccontextmanager
async def _lifespan(_app: FastAPI):
    # Best-effort, non-fatal warm-up: a real deployment wants the model
    # loaded before the first request pays for it, but a model-source
    # outage (or, in this sandbox, an egress policy blocking it) must
    # never crash the whole service — /health and /ready must still
    # respond so the rest of the pipeline (preprocessing, region
    # detection) keeps working, and /v1/providers/paddleocr/status
    # reports the real reason it isn't ready.
    try:
        registry.get().initialize()
    except Exception as exc:  # noqa: BLE001 — logged by the provider itself
        del exc
    yield


app = FastAPI(title="Milaserv360 Prescription OCR Service", version="0.1.0", lifespan=_lifespan)
...
@app.get("/ready")
def ready() -> dict:
    """Distinct from /health: a provider can be healthy (no fatal error)
    while its models are still loading or failed to load — /ready reports
    that honestly instead of collapsing it into a single status."""
    provider = registry.get()
    is_ready = provider.is_ready()
    return {"ready": is_ready, "provider": provider.name}


@app.get("/v1/providers")
def list_providers() -> dict:
    return {"providers": [registry.get(name).get_provider_info() for name in registry.names()]}


@app.get("/v1/providers/{name}/status")
...
@app.post("/v1/analyze-quality", response_model=AnalyzeQualityResponse, dependencies=INTERNAL_AUTH)
...
@app.post("/v1/detect-region", response_model=DetectRegionResponse, dependencies=INTERNAL_AUTH)
...
@app.post("/v1/preprocess", response_model=PreprocessResponse, dependencies=INTERNAL_AUTH)
...
@app.post(
    "/v1/detect-and-recognize", response_model=DetectAndRecognizeResponse, dependencies=INTERNAL_AUTH
)
def detect_and_recognize(req: DetectAndRecognizeRequest) -> DetectAndRecognizeResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=422, detail=f"OCR provider {provider.name!r} failed: {exc}") from exc
...
@app.post("/v1/detect-candidates", response_model=DetectCandidatesResponse, dependencies=INTERNAL_AUTH)
